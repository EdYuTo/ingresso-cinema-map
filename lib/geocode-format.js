...
(function (root, factory) {
  ...
  function formatGoogleAddressForGeocode(raw) {
    ...
    if (segments.length >= 2) {
      ...
      if (numNeighborhood) {
        ...
        // Typed input rarely repeats the city: "av faria lima 949, pinheiros".
        neighborhood = segments.pop();
      }
    }

    // "Avenida Paulista 1578" — the number rides along with the street name.
    if (!streetNumber && segments.length) {
      const inlineNumber = segments[segments.length - 1].match(/^(.*\S)\s+(\d+)$/);
      if (inlineNumber) {
        segments[segments.length - 1] = inlineNumber[1];
        streetNumber = inlineNumber[2];
      }
    }
  ...
  function pickGeocodeMatch(results, formatted) {
    ...
    const candidates = list.filter((item) => {
      ...
      const named = [address.city, address.town, address.village, address.municipality]
        .filter(Boolean);
      // `display_name` also carries the state, and São Paulo the state contains
      // Guarulhos — so trust it only when no city field came back at all.
      if (named.length) return named.some(name => normalizeForCompare(name).includes(wantedCity));
      return normalizeForCompare(item.display_name).includes(wantedCity);
    });
  ...
  /**
   * Neighbouring house numbers to probe when OSM has no node for the exact one.
   *
   * Closest candidates come first, crossing the street: OSM coverage is patchy
   * per side, and the nearest mapped neighbour of Faria Lima 949 is 952, on the
   * other side. A hit lands tens of metres away, versus the several hundred a
   * street-segment fallback can be off by.
   */
  function buildHouseNumberProbes(number, limit = 8) {
    const base = parseInt(String(number || '').trim(), 10);
    if (!Number.isFinite(base) || base <= 0) return [];

    const probes = [];
    for (let step = 1; probes.length < limit && step <= limit; step += 1) {
      for (const candidate of [base - step, base + step]) {
        if (candidate > 0 && probes.length < limit) probes.push(String(candidate));
      }
    }
    return probes;
  }
  ...
  /**
   * Whether an autocomplete pick still needs the ranked geocode path.
   *
   * Suggestions come straight from a free-text search, and a result with no
   * house number is one arbitrary segment of the road among several. Saved
   * addresses carry no geocoder details and are already exact.
   */
  function shouldRefineSuggestion(item, formattedTyped) {
    const address = item && item.address;
    if (!address || address.house_number) return false;
    return Boolean(formattedTyped && formattedTyped.number);
  }
