...
(function (root, factory) {
   ...
   *
   * Brazilian streets number odd and even sides separately, so stepping by two
   * stays on the same side. Closest candidates come first: a hit lands within a
   * few metres, versus the ~250 m a street-segment fallback can be off by.
   */
  function buildHouseNumberProbes(number, limit = 4) {
    const base = parseInt(String(number || '').trim(), 10);
    if (!Number.isFinite(base) || base <= 0) return [];

    const probes = [];
    for (let step = 2; probes.length < limit && step <= limit * 2; step += 2) {
